...
public class ProspectiveProfileTests
{
    private readonly ProspectiveProfile _profile = new();
    ...
    [Fact]
    public void Analyze_PlatformIsProspective()
    {
        var resume = BuildResume(ResumeFormatFlags.None, hasContact: true, includeCoreSections: true);

        var result = _profile.Analyze(resume);

        result.Platform.Should().Be(AtsPlatform.Prospective);
    }
