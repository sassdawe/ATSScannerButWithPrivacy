...
public static class ResumeTextAnalyzer
{
    private static readonly Dictionary<string, SectionType> SectionKeywords =
        new(StringComparer.OrdinalIgnoreCase)
        {
            ...
            // German / Swiss headers — several ATS platform profiles (e.g. prospective.ch,
            // digitalent) explicitly document recognising these alongside English equivalents.
            ["profil"] = SectionType.Summary,
            ["über mich"] = SectionType.Summary,
            ["zusammenfassung"] = SectionType.Summary,
            ["persönliches profil"] = SectionType.Summary,
            ["ziel"] = SectionType.Objective,
            ["berufsziel"] = SectionType.Objective,
            ["berufserfahrung"] = SectionType.Experience,
            ["berufserfahrungen"] = SectionType.Experience,
            ["arbeitserfahrung"] = SectionType.Experience,
            ["praktische erfahrung"] = SectionType.Experience,
            ["werdegang"] = SectionType.Experience,
            ["beruflicher werdegang"] = SectionType.Experience,
            ["ausbildung"] = SectionType.Education,
            ["weiterbildung"] = SectionType.Education,
            ["weiterbildung und ausbildung"] = SectionType.Education,
            ["schulbildung"] = SectionType.Education,
            ["akademischer werdegang"] = SectionType.Education,
            ["kenntnisse"] = SectionType.Skills,
            ["fähigkeiten"] = SectionType.Skills,
            ["kompetenzen"] = SectionType.Skills,
            ["fach- und methodenkompetenzen"] = SectionType.Skills,
            ["kenntnisse edv"] = SectionType.Skills,
            ["edv-kenntnisse"] = SectionType.Skills,
            ["fachgebiete"] = SectionType.Skills,
            ["zertifikate"] = SectionType.Certifications,
            ["zertifizierungen"] = SectionType.Certifications,
            ["projekte"] = SectionType.Projects,
            ["auszeichnungen"] = SectionType.Awards,
            ["publikationen"] = SectionType.Publications,
            ["ehrenamt"] = SectionType.Volunteer,
            ["freiwilligenarbeit"] = SectionType.Volunteer,
            ["sprachen"] = SectionType.Languages,
            ["sprachkenntnisse"] = SectionType.Languages,
            ["referenzen"] = SectionType.References,
        };
